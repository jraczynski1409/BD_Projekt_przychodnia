...
<body>
    ...
    <div id="con">
        <h1>Informacje o lekach</h1>
        <?php
           require_once "connect.php";
        
            $polaczenie = @new mysqli($host, $db_u, $db_p, $db_n);
        
            $zapytanie = "SELECT * from leki";
            
            $result=mysqli_query($polaczenie,$zapytanie);
            
            echo "<table class='tabela'>";
            
             echo "<tr class='naglowek_tabeli'><td>ID</td><td>Nazwa leku</td><td>Cena</td></tr>";
            
            while($row = mysqli_fetch_row($result))
            {
                echo "<tr><td>".$row[0]."</td>"."<td>".$row[1]."</td>"."<td>".$row[2].".99 PLN"."</td></tr>";
            }
            echo "</table>"
        ?>
        </div>
